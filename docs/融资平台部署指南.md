...
#### 公司联系人管理（新增）
- `PUT /api/admin/companies/:id/contact-person` - 更改公司联系人
  - 请求体：`{ "contact_person_id": "user-uuid" }` 或 `{ "contact_person_id": null }` 清除联系人
...
管理员可以更改公司的联系人：
- 在公司详情页面，可以查看当前联系人信息
- 通过搜索功能找到要设置为联系人的用户（支持按邮箱或姓名搜索）
- 可以设置任何用户（staff、investor、company）为公司联系人
- 可以清除联系人（设置为空）
- 联系人信息会显示在公司详情中

### 2. 权限管理（增强）
管理员可以为企业添加访问权限：
- **支持所有用户类型**：可以添加 staff、investor、company 角色的用户
- **搜索功能**：支持按邮箱或姓名搜索用户
- **权限类型**：view（查看）、review（审核）、full（完全权限）
- **批量管理**：可以批量添加权限、查看权限列表、删除权限
...
- **支持附件上传**：可以上传文件（PDF、DOC、XLS、PPT、图片、压缩包等，最多5个，每个最大10MB）
...
#### 企业回复功能
- 企业可以在自己的门户中查看管理员/Staff的反馈并回复
- **支持附件上传**：企业回复时也可以上传文件（最多5个，每个最大10MB）
- 所有附件都可以点击下载查看
...
- **contact_person_id**：公司联系人用户ID（可由管理员更改）
